...
# システムプロンプト
# ================================================================
_SYSTEM = """あなたは日本最高峰の技術提案スペシャリストであり、
...
1. 留意点・理由の対象となる工事の事実（工種・場所・地形・環境・使用機械・作業内容・
   ...
   作業内容（例：他工事で用いる特殊な工法・地域特有の環境要因など）は、この工事には
...
3. 「現場固有性」を出す唯一の正しい方法は、今回提供された施工条件プロンプト・
   ...
   周辺条件）を具体的に引用することであって、今回の資料に無い要素を盛り込むことではない。
4. もし施工条件プロンプト・資料に記載のない工種・環境要素があれば、留意点・理由に
   その要素を一切含めてはならない。その工事に実在する条件（地形・交通・近隣・
   地質・河川流量・仮設等）のみを根拠にすること。
...
6. 現場固有の状況（今回の施工条件プロンプト・設計図書に実際に書かれた地名・工種・
   数値・制約等）を理由の根拠に引用する。※今回の資料に無い工種・環境要素は
   一切用いない
...
- 【最重要の失格】今回の施工条件プロンプト・資料に書かれていない工種・環境・作業を、
  過去提案書や地域知識からの借用で混入させること。今回の資料に無い要素を出すのは即失格。

【過去提案書の扱い（文体・書き方の参考のみ／内容の流用は厳禁）】
過去提案書が提供された場合、参考にしてよいのは「書き方」だけであり、「内容」ではない：
1. 継承してよいのは、語彙選び・語尾・文体リズム・一文の構成の型・論理の運び方など
   「表現・書き方のスタイル」のみである。
2. 過去提案書に含まれる具体的な現場情報（地名・工種・作業内容・数値・
   環境条件・使用機械）は、今回の施工条件プロンプト・資料に同じ内容が実際に存在する
   場合を除き、一切引用・流用してはならない。過去が別現場の工事内容でも、今回が
...
0. 【最優先チェック】全ての留意点・理由の内容が、今回の施工条件プロンプト・資料に
   実際に書かれている事実のみに基づいているか。今回の資料に無い工種・環境・作業を、
   過去提案書や地域知識テンプレートから借用していないか。1つでも借用があれば、
   今回の資料に即した内容へ必ず書き直すこと（このチェックを他の全チェックに優先する）。
...
_SYSTEM_EVAL = """あなたは総合評価落札方式（簡易型B・対策なし型）における
...
- 地域特有の事情（気象・地形・周辺住民・交通・環境規制等）は、今回の工事が実際に
  それに該当する場合にのみ加点要素となる。
- 【厳守】今回の資料に記載のない事情への言及が無いことを理由に減点してはならない。
  逆に、今回の資料に無い工種・環境要素が混入している場合は「別現場の内容の混入」として
...
def _build_ref_block(reference_context: str) -> str:
    ...
    return (
        ...
        "以下は過去に提出した提案書です。参考にするのは「書き方（語彙・\n"
        "語尾・文体リズム・一文の構成の型）」だけであり、そこに書かれた具体的な現場情報\n"
        "（地名・工種・作業内容・数値・環境条件）は今回の工事の事実ではありません。\n"
...
def _build_evolve_note(reference_context: str) -> str:
    ...
    return (
        ...
        "・過去提案書の現場情報（地名・工種・作業内容・数値・環境）は流用しない。\n"
...
def _create_message(client: "anthropic.Anthropic", user_msg: str, max_tokens: int = 16000,
    ...
    images を渡した場合（[{"media_type": "image/jpeg", "data": "<base64>"}, ...]）、
    現場写真等を画像入力としてテキストと併せて送る（generate_prompt_from_docs用）。
    """
...
def _generate_notes_single(
    ...
    user_msg = (
        ...
        f"・【厳守】今回の資料に書かれていない工種・環境・作業を過去提案書や\n"
        f"  一般知識から補ってはならない。今回の資料に無い要素は一切書かないこと。\n"
        f"{evolve_note}\n\n"
        f"JSON配列のみで回答（```不要）:\n"
...
def generate_all_reasons(item_labels: list, all_notes: dict,
    ...
    user_msg = (
        ...
        f"・【厳守】今回の資料に無い工種・環境・作業を過去提案書や一般知識から\n"
        f"  補って理由に混入させないこと。今回の資料に無い要素の根拠は一切書かないこと。\n\n"
...
def generate_alternative_notes(
    ...
    user_msg = (
        ...
        f"・【厳守】今回の資料に無い工種・環境・作業を過去提案書や一般知識から\n"
        f"  補ってはならない。今回の資料に無い要素は一切書かないこと。\n"
        f"・JSONの配列のみで回答（説明文・```不要）\n\n"
        f'["留意点①テキスト", "留意点②テキスト", ...]'
    )
...
def generate_prompt_from_docs(docs_context: str, project_name: str = "",
    ...
    """
    工事資料（および現場写真）を解析して施工条件プロンプトを自動生成する。
    施工条件プロンプトは後続の留意点・理由生成の精度を左右する最重要の入力情報のため、
    (1) アップロードされた資料からの詳細抽出を最優先しつつ、
    (2) 施工前の現場写真・周辺写真・気になる写真等が添付されていれば、それを画像として
        直接読み取り、資料だけでは分からない現場の実際の状況（地形・狭隘度・近接構造物・
        路面状態・仮設状況・気になる支障物等）を抽出し、
    (3) Web検索ツールで類似工事の事例・現場周辺の地域特性（気象・地形・交通・環境規制等）を
        調査し、資料・写真から得た事実を補強・具体化したうえで、
    (4) 情報密度が最も高いプロンプトを毎回組み立てる。

    images: [{"media_type": "image/jpeg", "data": "<base64>"}, ...]
            施工前の現場写真・周辺状況の写真・気になる箇所の写真等（jpg/png、HEICは
            呼び出し側で変換済み前提）。
    """
    key = api_key or _load_api_key()
    client = anthropic.Anthropic(api_key=key, timeout=300.0)
    ...
    docs_block = _build_docs_block(docs_context)
    photo_block = (
        f"【現場写真】施工前の現場・周辺状況・気になる箇所の写真が{len(images)}枚添付されています。\n"
        f"写真に写っている実際の状況（地形・勾配、道路幅員や交通量、近接する住宅・構造物・\n"
        f"河川・農地等、資材置場や仮設の状況、施工上の支障物・気になる箇所等）を注意深く\n"
        f"観察し、文章資料には書かれていない具体的な現場情報として抽出・活用すること。\n\n"
        if images else ""
    )

    user_msg = (
        f"以下の工事資料{'・現場写真' if images else ''}を読み込み、技術提案書（様式４）の留意点生成に使う施工条件プロンプトを作成してください。\n\n"
        f"{project_block}"
        f"{docs_block}\n"
        f"{photo_block}"
        f"【作業手順】\n"
        ...
        + (
            f"2. 添付された現場写真を観察し、資料には明記されていない現場固有の視覚情報\n"
            f"   （地形・狭隘度・近接構造物・路面や地盤の状態・仮設状況・気になる支障物等）を\n"
            f"   抽出すること。写真から読み取れない・判断できないことは推測で補わないこと。\n"
            if images else ""
        )
        + f"{'3' if images else '2'}. 次に、Web検索ツールを使って以下を徹底的に調べること：\n"
        ...
        f"   関連情報にあたること。\n"
        f"{'4' if images else '3'}. 資料{'・写真' if images else ''}から得た事実（最優先）と、検索で得た同種工事の一般的な留意点・地域特性\n"
        ...
        f"  それを混入させない）。\n"
        + (
            f"・現場写真から読み取った内容は、あくまで実際に写真に写っているものだけを事実として\n"
            f"  扱うこと。写真に写っていないものを推測で書き加えないこと。\n"
            if images else ""
        )
    ...
    try:
        msg = _create_message(client, user_msg, model=model, tools=_WEB_SEARCH_TOOL, images=images)
    except anthropic.RateLimitError:
        raise RuntimeError("APIのレート制限に達しました。しばらく待ってから再試行してください。")
